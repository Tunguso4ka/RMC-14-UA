cm-job-name-xeno-boiler = Кип'ятильник
CMJobXenoBoiler = Кип'ятильник

cm-job-name-xeno-burrower = Нюхач
CMJobXenoBurrower = Нюхач

cm-job-name-xeno-carrier = Перевізник
CMJobXenoCarrier = Перевізник

cm-job-name-xeno-crusher = Дробарник
CMJobXenoCrusher = Дробарник

cm-job-name-xeno-defender = Захисник
CMJobXenoDefender = Захисник

cm-job-name-xeno-drone = Дрон
CMJobXenoDrone = Дрон

cm-job-name-xeno-hivelord = Володар вулика
CMJobXenoHivelord = Володар вулика

cm-job-name-xeno-hugger = Лицехват
CMJobXenoHugger = Лицехват

cm-job-name-xeno-larva = Личинка
CMJobXenoLarva = Личинка

cm-job-name-xeno-lesser-drone = Малий дрон
CMJobXenoLesserDrone = Малий дрон

cm-job-name-xeno-lurker = Ховака
CMJobXenoLurker = Ховака

cm-job-name-xeno-praetorian = Преторіанець
CMJobXenoPraetorian = Преторіанець

cm-job-name-xeno-queen = Королева
CMJobXenoQueen = Королева

cm-job-name-xeno-ravager = Розорювач
CMJobXenoRavager = Розорювач

cm-job-name-xeno-runner = Бігун
CMJobXenoRunner = Бігун

cm-job-name-xeno-sentinel = Вартовий
CMJobXenoSentinel = Вартовий

cm-job-name-xeno-spitter = Плювака
CMJobXenoSpitter = Плювака

cm-job-name-xeno-warrior = Воїн
CMJobXenoWarrior = Воїн

# TODO CM14
department-CMXeno = Ксеноморфи
cm-department-Xeno-description = ""

cm-job-name-selectable-xenomorph = Ксеноморф
CMXenoSelectableXenomorph = Ксеноморф

cm-job-supervisors-queen = Королева

role-timer-xeno-roles = Ксеноморф
role-timer-xeno-drones = дрон та еволюції дрона
role-timer-xeno-tier-three = касти третього рівня
